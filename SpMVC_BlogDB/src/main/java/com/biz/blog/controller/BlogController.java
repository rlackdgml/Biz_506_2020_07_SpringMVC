...
public class BlogController {
	...
	// http://localhost:8080/blog/blog/list 주소로 reg했을때
	// 응답할 함수

	// method= RequestMethod.GET : reg 를 할떄 <a href=주소>를 클릭했을때
	// 응답하는 method
	...
	public String input(Model model) {  //작성하기
		
		// write.jsp에서 input POST로 데이터를 전달할때
		// 비어잇는("")데이터 떄문에 발생하는 400 오류를 방지하기 위해
	...
	public String write(@ModelAttribute BlogVO blogVO, Model model) { // 저장하기

		/*
		 * 
		 * Debuging Code : 어떤 값을 확인하는 용도 form에서 건너온 뎅디터가 정확히 VO에서 담겼는가를 확인하기 위해 사용한 코드 이
...
//		model.addAttribute("USER",blogVO.getBl_user());
	...
	public String view(String seq, Model model) { // 이 매개변수를 통해 리스트에서 view함수를 호출함 (seq)

		log.debug("SEQ : {}", seq);

		/*
		 * SQL Injection공격을 사전에 차단하기 위해 Controller에서 SEQ값을 문자열형에서 Long형으로 변환하는 코드를 추가
		 */
	...
	public String update(BlogVO blogVO, Model model) {
		...
		// + blogVO.getBl_seq(); 자동으로 만들어준다

		// 수정이 완료되면 다시 detail view 로 화면을 전호나하기
