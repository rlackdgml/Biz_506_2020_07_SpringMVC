...
public class HelloRestController {
	 ...
	 * 컨테이너가 있으며
	 * 
	 * 컨터이너에 저장된 객체를 찾아서 선언된 객체에 주입하여 초기화, 사용할수 있도록 만들어준다
	...
	*/
	 ...
	 * 
	 * 프로젝트가 시작될떄 @Annotation이 붙어있는 모든 클래스는 이미 객체로 생성되어
	 * Container 에 저장되어있다.
	 * 
	 ...
	 * 
	 */
	//public HelloRestController(HelloService hService, ScoreService sService) { // 매서드 안 에 전달 해야 한다
	//	this.hService = hService;                                                           //톰켓이 프로젝트를 대체를한다 온어테이션이 붙어있는 객체들가져온다 ioc
	...
	public String rest(Model model) {
		...
		return "Republic of Korea";  // view가 없어도됨 그냥 이문자열을 그대로 보여줘라 (Restfull)
	}
	...
	public String mNull(Model model) {
		...
		return null;   // 의미없는 값을 선언해도 오류를 보여주지않는다.  
	}
	
	@RequestMapping(value="/int")
	public String hello() {
		int ret = hService.add(20,40); //hService 가 사용할준비가 안됨
		return ret + "";
	}
