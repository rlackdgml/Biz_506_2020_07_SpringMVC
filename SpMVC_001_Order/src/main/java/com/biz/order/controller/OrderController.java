...
public class OrderController {
	...
	public String getOrder(Model model) {
		...
		// orderVO 에 담는다.
		OrderVO orderVO = oService.findBySeq(44);
		
		/*
		 * view(*.jsp)파일에 전달하여 Rendering을 수행할 수 있도록
		 * model 객채에 orderVO 데이터를 추가해 놓는다.
		 */
		model.addAttribute("ORDER", orderVO);
		
		/*
		 * Dispatcher야 /WEB-UNF/views/order/view.jsp 파일을 읽어서
		 * 클라이언트(요청한곳)으로 응답을 하여라
		 * 이때 model에 담겨잇는 데이터를 사용하여 rendering 할떄 사용하라
		 */
		return "order/view";
