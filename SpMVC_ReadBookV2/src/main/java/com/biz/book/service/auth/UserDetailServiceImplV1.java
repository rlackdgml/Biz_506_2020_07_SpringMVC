 ...
 * 
 * Customizing 
 * 패키지형 솔루션을 가지고 있는 it 회사ㅓ에서
 * 어떤 회사에 솔루션을 판매하면서 
 * 회사의 실정, 업무환경, 열러가지 여건들을 요구분석하여
...
public class UserDetailServiceImplV1 implements UserDetailsService {
	 ...
	 *  @Autowired로 주입받은 객채에 메모리 누수현상이 발생을 하더라
	 *  
	 *  주입받을 객체로 final로 선언을 해주는데
	 ...
	 *  
	 *  auth 패키지는 서브릿  USERimplV1은 컨테스트 실행
	 */
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		*/
		...
		if(userDetail == null) {
			...
			// 강제로 UsernameNotFoundException을 발생시킨다.
		 ...
		 * email 인증과 같은 절차를 통과했을때 이 값을 true로 만들어서
		 * 로그인이 될수 있도록 설정
		 */
	...
	// tbl_authority 로부터 ROLE 정보를 가져와서 List로 생성
	private Collection<GrantedAuthority> getAuthorities(String username){
		
		// tbl_authority 로 부터 ROLE 정보를 SELECT
		...
		for(AuthorityVO vo : authList) {
			...
			// 리스트로 생성하기
			// SimpleGranteAuthority 클래스를 사용하여 변환
