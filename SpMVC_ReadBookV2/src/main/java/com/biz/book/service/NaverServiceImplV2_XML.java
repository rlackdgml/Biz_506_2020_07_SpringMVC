...
public class NaverServiceImplV2_XML extends NaverServiceimplV1 {
	...
	// 여기에 아무런 코드가 없어도 프로젝트가 정상 수해잉 되고 있다.
	...
	public List<BookVO> getNaverList(String queryURL) {
		...
		/*
		 * springframework에서 외부 API를 사용하여 데이터를 가져올때 기존에는 JSON(XML) 형식으로 가져오고, 여러가지 외부
		 * 라이브러리를 사용하여 객체로 parsing하는 과정을 복잡하게 만들어야 했다.
		 * 
		 * spring에서 외부 API를 사용하여 데이터를 가져올일이 점점 많아 지면서 RestTempate라는 클래스를 새로 만들어
		 * framework 프로젝트에서 사용할수 잇도록 만들어 두었다.
		 * 
		 * HttpHeader, HttpEntity, ResponseEntity 객체만 잘 작성하면 외부 API에 요청하고, 응답받은 데이터를
		 * parsing 하는 절차를 대신 수행 해준다.
		 */
