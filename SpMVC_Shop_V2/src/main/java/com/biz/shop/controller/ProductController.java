...
public class ProductController {
	 ...
	 * URL에 담긴 변수이름과 실제 사용하는 파라메티 변수가 다를때는
	 * @RequestParam()을 사용하는 방법이 있다.
	 * @RequestParam()은 spring4.x 이상에서는 선택사항이다.
	...
	public String update(@RequestParam("id") String p_code,
						@ModelAttribute("PRO_VO") ProductVO proVO,Model model) { // 생성해야함
	...
	public String update(@ModelAttribute ProductVO proVO,Model model) {
		...
		return "redirect:/product/detail"; // ?ide= "+proVO.getP_code();
