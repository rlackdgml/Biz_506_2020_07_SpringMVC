...
public class NaverMailSendService {
	...
	public void sendMail(EmsVO emsVO) {
		 ...
		 * Hyper Text markup tag로 만들어진 문서를 HTML DOC type이라고 하며 이문서는 Web Browser를 통해서 내용을
		 * 확인할수 있다. 이처럼 Mime type을 지정해둔 문서는 인터넷으로부터 받았을때 즉시 내용을 확인할수 있다.
		 * 
		 * xMail을 통해서 지금 만드는 Mime type은 email viewer 에서 확인할 수 있는 문서 형식이다. 이 문서는 메모장이나,
		 * WebBrowser로 보면 그내용을 정확히 확인이 어렵다. 대표적으로 xMail Mime type 문서를 볼수 있는 어플리케이션에 아웃룩이
		 * 있고, 각 스마트기기의 고유 Email App이 이런 종류에 해당한다.
		 * 
		 */
