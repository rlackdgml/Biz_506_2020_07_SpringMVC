...
public class HomeController {
	...
	// public String write(String from_email, String to_email, String
	// s_subject,Model model, EmsVO emsVO) {
	public String write(@ModelAttribute EmsVO emsVO,
			@RequestParam(value = "file1", required = false) MultipartFile file1,
			@RequestParam(value = "file2", required = false) MultipartFile file2,
			// 파일을 첨부하지 않아도 오류를 막을수 있다.
			Model model) {
...
//      model.addAttribute("EMS",emsVO);
	...
	/*
	 * 목록보기(home)에서 제목을 클릭하면 id값을 변수로 넘기면서 update, GET 로 호출이된다. id값으로 findById()조회를
	 * 해서 EmsVO를 DB로 부터 Select하고 그 결과를 model에 담아서 다시 ems-write.jsp로 보내기
	 */
	...
	public String update(EmsVO emsVO, @RequestParam(value = "file1", required = false) MultipartFile file1,
		...
		// 4. upload 된 file2 가 있는지 검사해서 emsVO에 setting
		if (!file2_ex) {
